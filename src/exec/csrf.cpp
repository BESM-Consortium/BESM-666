...
CSRF::CSRF() : mstatus(*this), mepc(*this), privillege_(PRIVILLEGE_MACHINE) {}

std::variant<bool, RV64UDWord> CSRF::write(RV64UDWord rawId, RV64UDWord value) noexcept {
...
PrivillegeLevel CSRF::registerPrivillege(ICSR::Id id) noexcept {
    RV64UWord rawid = static_cast<RV64UWord>(id);
    RV64UWord rawpriv =  util::ExtractBits<RV64UWord, 2, 8>(rawid);
    return static_cast<PrivillegeLevel>(rawpriv);
}
